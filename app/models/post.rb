class Post < ActiveRecord::Base
  ...
  scope :not_empty,      -> { joins(:raw_post).where("(raw_posts.post->>'story') ISNULL OR NOT (raw_posts.post->>'story') SIMILAR TO ? AND NOT (raw_posts.post->>'story') = ?", "%(אוהב|הגיב|אהב|את תמונת הנושא|ה‏תמונה‏ שלו/שלה)%", "‎‎") }
